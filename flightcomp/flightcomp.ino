...
void loop() 
{
    ...
    unsigned long interval = 9620;  // microseconds for ~104 Hz
...
void posfuser()
{
  ...
  float mx = magEvent.magnetic.x; float my = magEvent.magnetic.y; float mz = magEvent.magnetic.z;
...
// gpx, gyro position axis
    gpx += (gax / 104);
      if (abs(gpx) >= 360){
        gpx = 0; //Rollover if compeleted rotation
      }
    gpy += (gay / 104);
      if (abs(gpy) >= 360){
        gpy = 0; 
    ...
    Serial.print(gpx); Serial.print('\t');
    Serial.print(gpy); Serial.print('\t');
    Serial.println(gpz);
    
    //Deda reckoning of Aceel data 
    // accel distance axis
//Corect Accel adata for grvity
